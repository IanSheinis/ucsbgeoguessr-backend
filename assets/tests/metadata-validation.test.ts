...
import { imgConfig } from '../metadata/images';
...
describe('Asset and Metadata Validation', () => {
  ...
  test('Metadata must match the required JSON structure and constraints', () => {
    ...
    imgConfig.forEach((meta: any, index) => {
      ...
      try {
        const parsedCats = JSON.parse(meta.Categories);
        if (!Array.isArray(parsedCats) || !parsedCats.every(c => typeof c === 'string')) {
          throw new Error();
        }
      } catch (e) {
        errors.push(`Metadata [${identifier}]: 'Categories' must be a JSON string of an array (e.g. '["A", "B"]')`);
      }
  ...
  test('File constraints (Size check)', () => {
    ...
    imageFiles.forEach(file => {
      ...
      if (!imgConfig.find(m => m.imgName === file)) {
        errors.push(`Image '${file}' has no entry in images.ts`);
      }
    });
