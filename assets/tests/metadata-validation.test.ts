    ...
    test('All files in assets/images must be valid image types', () => {
        ...
        imageFiles.forEach((file) => {
            ...
            if (!VALID_IMAGE_EXTENSIONS.includes(ext)) {
                errors.push(
                    `Invalid file found: '${file}'. Only ${VALID_IMAGE_EXTENSIONS.join(', ')} are allowed.`,
                );
            }
        });
        if (errors.length > 0) {
    ...
    test('Metadata must match the required JSON structure and constraints', () => {
        ...
        imgConfig.forEach((meta: any, index) => {
            ...
            if (meta.imgName && !imageFiles.includes(meta.imgName)) {
                errors.push(
                    `Metadata [${identifier}]: File '${meta.imgName}' not found in assets/images (Check extensions!)`,
                );
            }
            ...
            if (
                ...
                !meta.Categories.every((c: any) => typeof c === 'string')
            ) {
                errors.push(
                    `Metadata [${identifier}]: 'Categories' must be an array of strings (e.g. ["A", "B"])`,
                );
            }

            if (!isValidCoordinate(meta.Latitude, -90, 90)) {
                errors.push(
                    `Metadata [${identifier}]: Latitude '${meta.Latitude}' is out of range (-90 to 90)`,
                );
            }
            if (!isValidCoordinate(meta.Longitude, -180, 180)) {
                errors.push(
                    `Metadata [${identifier}]: Longitude '${meta.Longitude}' is out of range (-180 to 180)`,
                );
            }
        });
        if (errors.length > 0) {
